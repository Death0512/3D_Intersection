...
def test_generate_keeps_vehicles_that_continue_past_clip_end():
    """A traffic clip is a time window, not a closed trip experiment.

    Vehicles generated by demand must not be discarded merely because their
    planned leave_frame is after duration_frames. Short dense/adaptive Kaggle
    clips can otherwise become empty even though arrivals were generated.
    """
    import tempfile
    dm = S.DemandModel(
    ...
    assert any(v["depart_frame"] < scn["duration_frames"]
               for v in scn["vehicles"])


def test_free_flow_speed_range_allows_80_kmh():
    assert S.SPEED_KMH_RANGE == (30, 80)
