...
#   --seconds F         Video length in seconds (default: 12), hard ceiling.
#                       Vehicles are generated by demand; unfinished vehicles
#                       are discarded so render time is predictable.
#   --out DIR           Output directory (default: output/run_car)
#   --north-scenarios   Run the six SUMO North-camera scenarios sequentially.
#                       Use with --seconds/--fps/--samples/--seed/--out.
...
#                        flow veh/h + turning split). Default: built-in demand
...
#   --demand-profile P   SUMO-only time-varying demand profile, e.g.
...
#                       Cap on Blender workers per GPU (default: 1). One worker
#                       per GPU avoids Cycles/OptiX context contention.
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --north-scenarios) NORTH_SCENARIOS=1; shift ;;
        --only)           ONLY="$2";          shift 2 ;;
...
if [[ "$NORTH_SCENARIOS" -eq 1 ]]; then
    BATCH_ARGS=(
        --seconds "$SECONDS_VAL"
        --fps "$FPS"
        --seed "$SEED"
    )
    [[ -n "$OUT_DIR" ]] && BATCH_ARGS+=(--out "$OUT_DIR")
    [[ -n "$ONLY" ]] && BATCH_ARGS+=(--only "$ONLY")
    [[ "$SAMPLES" -gt 0 ]] && BATCH_ARGS+=(--samples "$SAMPLES")
    [[ "$SKIP_ASSET_CHECK" -eq 1 ]] && BATCH_ARGS+=(--skip-asset-check)
...
echo "============================================================"
if [[ "$IS_KAGGLE" -eq 1 ]]; then
    echo "PIPELINE [Kaggle]  seed=$SEED  seconds=${SECONDS_VAL}s @ ${FPS}fps  out=$OUT_DIR"
else
    echo "PIPELINE  seed=$SEED  seconds=${SECONDS_VAL}s @ ${FPS}fps  out=$OUT_DIR"
fi
